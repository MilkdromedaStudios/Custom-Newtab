...
function bindGlobalEvents() {
  ...
  $('#copy-page-url').addEventListener('click', copyPageUrl);
...
async function copyPageUrl() {
  const pageUrl = globalThis.chrome?.runtime?.getURL ? chrome.runtime.getURL('newtab.html') : location.href;
  if (navigator.clipboard?.writeText) {
    await navigator.clipboard.writeText(pageUrl);
  } else {
    prompt('Copy this extension page URL', pageUrl);
  }
  $('#copy-page-url').textContent = 'Copied URL';
  setTimeout(() => { $('#copy-page-url').textContent = 'Copy page URL'; }, 1600);
}
